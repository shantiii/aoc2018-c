...
void solve_problem5(FILE *input) {
	...
	while ((read = fgetc(input)) != EOF && isalpha(read)) {
		...
		while (stack.size > 1 && compatible(((int *)stack.data)[stack.size-1], ((int *)stack.data)[stack.size-2]) == 1) {
	...
	for (char taboo = 'a'; taboo <= 'z'; ++taboo) {
		...
		for (size_t i = 0; i < stack.size; ++i) {
			...
			if (dst == taboo || dst - 'A' + 'a' == taboo) { continue; }
			vector_push(&p2_stack, &dst);
			while (p2_stack.size > 1 && compatible(((char *)p2_stack.data)[p2_stack.size-1], ((char *)p2_stack.data)[p2_stack.size-2]) == 1) {
