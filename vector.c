...
void
vector_init (struct vector *v, size_t width, size_t capacity)
{
  if (v)
    {
      v->size = 0;
...
size_t
vector_width (const struct vector *v)
{
  return v->width;
}
...
void
vector_at (const struct vector *v, size_t index, void *elem)
{
  if (v->size > 0 && elem)
    {
...
void
vector_push (struct vector *v, const void *elem)
{
...
void
vector_pop (struct vector *v, void *elem)
{
  if (v->size > 0)
    {
      v->size--;
	...
	{
	  (void) memcpy (elem,
			 ((char *) v->data) + v->width * v->size, v->width);
	}
    }
}
